        ...
        with self._lock:
            ...
            if entry is None and identity_supplied:
                ...
                if available_count <= 1:
                    # There is no alternate credential to rotate to. Returning
                    # the only pool entry would report a successful recovery
                    # and retry the same rejected credential indefinitely.
                    self._unmatched_rotation_streak = 0
                    self._current_id = None
                    return None
