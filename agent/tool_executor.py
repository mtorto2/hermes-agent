            ...
            try:
                def _execute(next_args: dict) -> Any:
                    return _ra().handle_function_call(
                        ...
                        skip_tool_execution_middleware=True,
                        tool_request_middleware_trace=list(middleware_trace),
                        # Thread the active agent through the production dispatch
                        # seam so kanban/worker completions capture real runtime
                        # usage (Matt-local kanban run usage receipts).
                        agent=agent,
