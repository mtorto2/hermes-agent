...
def _session_owns_durable_lifecycle(
    session_id: str | None, *, session: dict | None = None
) -> bool:
...
def _session_async_delegation_selectors(
    ...
    session_id = getattr(agent, "session_id", None) or session_key
    owned_session_key = session_key if _session_owns_durable_lifecycle(
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    try:
        ...
        if fallback_notice is not None:
            try:
                cfg = cfg if cfg is not None else load_config()
                cli_tools = set(((cfg.get("platform_toolsets") or {}).get("cli") or []))
                if cli_tools:
                    print(fallback_notice, file=sys.stderr, flush=True)
                    return sorted(cli_tools | {"apple_calendar", "kanban", "project"})
            except Exception:
                pass
            print(
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        "clarify_callback": lambda q, c, multi_select=False: _block_with_light_cue(
            "clarify.request",
            sid,
...
def _notification_poller_loop(
    ...
    while not stop_event.is_set() and not session.get("_finalized"):
        _now = time.monotonic()
        if _now >= next_slot_retry_at:
            _retry_tui_light_slot_registration(session)
            next_slot_retry_at = _now + _TUI_LIGHT_SLOT_RETRY_INTERVAL_S
