...
| `/queue <prompt>` / `/q <prompt>` | Queue a prompt for the next turn (doesn't interrupt the current agent response). Use `/q delete` to delete the only queued prompt, or `/q delete <number>` when multiple prompts are queued. |
...
| `/status` | Show session info — model, provider, profile, session ID, working directory, title, created/updated timestamps, token totals, agent-running state, queue depth, and connected platforms. |
...
| `/status` | Show session info, including session ID, title when set, timestamps, token total, running state, queue depth, and connected platforms. |
