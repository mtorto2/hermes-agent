...
def test_non_local_creation_uses_the_raw_session_cwd(monkeypatch):
    """A first vision read must seed the shared sandbox from its own session."""
    monkeypatch.setenv("TERMINAL_ENV", "ssh")
    task_id = "acp-session"
    eff = tt._resolve_container_task_id(task_id)
    ...
    fake = SimpleNamespace(execute=lambda *a, **k: {"returncode": 0, "output": ""})
    try:
        tt.register_task_env_overrides(task_id, {"cwd": "/workspace/acp-project"})
        with patch.object(tt, "_create_environment", return_value=fake) as create:
            assert tt.ensure_task_env(task_id) is fake
        assert create.call_args.kwargs["cwd"] == "/workspace/acp-project"
    finally:
        _clear(eff, task_id)
