...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            # /tmp resolves to /private/tmp on macOS; the production guard
            # deliberately requires the canonical temp-directory spelling.
            temp_dir = Path(approval_module.tempfile.gettempdir()).resolve()
            for prefix in ("hermes-verify-", "hermes-ad-hoc-"):
                assert detect_dangerous_command(f"rm -f {temp_dir / f'{prefix}example.py'}") == (
                    False,
                    None,
                    None,
                )
