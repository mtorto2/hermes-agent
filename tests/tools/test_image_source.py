...
class TestLazySandboxBringUp:
    ...
    @pytest.mark.asyncio
    async def test_container_read_uses_the_calling_sessions_cwd(self, tmp_path, monkeypatch):
        """A shared sandbox must not resolve a relative image in another session's cwd."""
        isrc = _reload(monkeypatch, tmp_path / "hermes")
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        ...
        tt.record_session_cwd("acp-session", "/workspace/acp-project")
        calls = {}
